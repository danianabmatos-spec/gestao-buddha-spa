// Script para testar o relatório de Vendas da Recepção
import { config } from 'dotenv'
import { fileURLToPath } from 'url'
import { dirname, join } from 'path'

const __dirname = dirname(fileURLToPath(import.meta.url))
config({ path: join(__dirname, '.env.local') })

const BASE_URL = 'https://app.bellesoftware.com.br/api/release/controller'

const HEADERS = {
  'Content-Type': 'application/json',
  Accept: 'application/json, text/plain, */*',
  Origin: 'https://app.bellesoftware.com.br',
  Referer: 'https://app.bellesoftware.com.br/',
  'x-from': 'app',
  'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/148.0.0.0 Safari/537.36',
}

async function getToken() {
  const email = process.env.BELLE_METROPOLE_EMAIL
  const senha = process.env.BELLE_METROPOLE_PASSWORD

  const resp = await fetch(`${BASE_URL}/Login/v1.0/autenticar`, {
    method: 'POST',
    headers: HEADERS,
    body: JSON.stringify({
      metodo: 'email',
      dados: { email, senha }
    }),
  })

  if (!resp.ok) {
    const txt = await resp.text()
    throw new Error(`Autenticação falhou: ${resp.status} - ${txt}`)
  }

  const data = await resp.json()
  const token = data.token

  // Chamadas adicionais necessárias
  await fetch(`${BASE_URL}/Login/v1.0/admin/recuperar_dados?estabGeral=`, {
    headers: { ...HEADERS, Authorization: token },
  })
  await fetch(`${BASE_URL}/Login/v1.0/gravarsessao?estabGeral=`, {
    method: 'POST',
    headers: { ...HEADERS, Authorization: token },
    body: JSON.stringify({}),
  })

  return token
}

async function buildReport(token, reportId, dataIni, dataFim) {
  const payload = {
    reportId,
    sortColumn: null,
    sortOrder: 1,
    estab: '1',
    ignoreRecords: false,
    filters: [
      {
        id: 1,
        field: { type_id: 'data', description: 'Período' },
        operator: { allow_multiple_values: true, description: 'Entre', id: null },
        value: dataIni,
        value2: dataFim,
      },
    ],
  }

  const resp = await fetch(`${BASE_URL}/BI/v1.0/report/build?estabGeral=1`, {
    method: 'POST',
    headers: { ...HEADERS, Authorization: token },
    body: JSON.stringify(payload),
  })

  if (!resp.ok) {
    const txt = await resp.text()
    throw new Error(`Report ${reportId} falhou: ${resp.status} - ${txt.slice(0, 200)}`)
  }

  return resp.json()
}

async function main() {
  console.log('🔐 Autenticando...')
  const token = await getToken()
  console.log('✅ Token obtido\n')

  // Período de teste: Junho 2026 (até hoje)
  const dataIni = '2026-06-01'
  const dataFim = '2026-06-14'

  console.log(`📅 Período: ${dataIni} a ${dataFim}\n`)

  // Primeiro, tentar listar todos os relatórios disponíveis
  console.log('📋 Tentando listar relatórios disponíveis...\n')
  try {
    const listResp = await fetch(`${BASE_URL}/BI/v1.0/report/list?estabGeral=1`, {
      headers: { ...HEADERS, Authorization: token },
    })
    if (listResp.ok) {
      const reports = await listResp.json()
      console.log('✅ Lista de relatórios obtida!')
      console.log(JSON.stringify(reports, null, 2).slice(0, 2000))

      // Procurar por relatórios com "comissão" ou "vendas"
      if (Array.isArray(reports)) {
        const vendasReports = reports.filter(r =>
          r.name?.toLowerCase().includes('comissão') ||
          r.name?.toLowerCase().includes('vendas') ||
          r.name?.toLowerCase().includes('venda')
        )
        console.log('\n🎯 Relatórios de vendas/comissão encontrados:')
        vendasReports.forEach(r => {
          console.log(`   ID ${r.id}: ${r.name}`)
        })
      }
    }
  } catch (error) {
    console.log('❌ Não foi possível listar relatórios')
  }

  // Testar Report IDs conhecidos e alguns possíveis para Comissão de Vendas
  const reportIdsParaTestar = [
    188, // Próximo após 187
    189,
    190,
    191,
    192,
    193,
    194,
    195,
  ]

  for (const reportId of reportIdsParaTestar) {
    try {
      console.log(`\n📊 Testando Report ID ${reportId}...`)
      const result = await buildReport(token, reportId, dataIni, dataFim)

      console.log(`✅ Report ${reportId} funcionou!`)
      console.log(`   Colunas: ${result.columns?.map(c => c.title).join(', ')}`)
      console.log(`   Total de registros: ${result.record_count}`)
      console.log(`   Totalizações: ${result.totalization_data?.length || 0}`)

      if (result.totalization_data?.length > 0) {
        console.log('\n   📈 Totalizações encontradas:')
        result.totalization_data.forEach(tot => {
          console.log(`      - ${tot.label}: ${tot.value}`)
        })
      }

      if (result.data?.length > 0) {
        console.log(`\n   📋 Primeiras linhas de dados:`)
        console.log('   ', JSON.stringify(result.data.slice(0, 2), null, 2))
      }

      // Se encontramos algo que parece ser de vendas, salvar para análise
      const nomesColunas = result.columns?.map(c => c.title.toLowerCase()).join(' ') || ''
      if (nomesColunas.includes('venda') || nomesColunas.includes('comissão') ||
          nomesColunas.includes('voucher') || nomesColunas.includes('produto')) {
        console.log(`\n   🎯 ESTE PODE SER O RELATÓRIO DE VENDAS!`)
      }

    } catch (error) {
      console.log(`   ❌ Report ${reportId} falhou: ${error.message.slice(0, 100)}`)
    }
  }
}

main().catch(console.error)
