...
export async function GET() {
  ...
  const lista = usuarios.map((u) => {
    ...
    if (u.perfil === 'RECEPCAO' && u.unidade) uns = [u.unidade]
  ...
  return Response.json({ usuarios: lista, unidades, meuId: session.sub })
}
...
export async function POST(req: NextRequest) {
  ...
  const perfil = (PERFIS.includes(b.perfil) ? b.perfil : 'RECEPCAO') as Perfil
  ...
  if ((perfil === 'RECEPCAO' || perfil === 'COORDENACAO') && unidadeIds.length === 0) {
  ...
  const unidadeId = perfil === 'RECEPCAO' ? unidadeIds[0] : null
