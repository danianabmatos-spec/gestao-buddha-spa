...
export function HistoricoChart({ historico }: HistoricoChartProps) {
  ...
  const tendencia = calcularTendencia(meses.map((mes, i) => ({
    x: i,
    y: historico.find(h => h.mes === mes && h.ano === 2026)?.caixa ?? null,
  })))
  ...
  const chartData = meses.map((mes, i) => {
    ...
    anos.forEach(ano => {
      const valor = historico.find(h => h.mes === mes && h.ano === ano)?.caixa || null
      dataPoint[`${ano}`] = valor
    })
    return dataPoint
  })
  ...
  const todosValores = historico.map(h => h.caixa).filter(v => v > 0)
  ...
  const yMin = Math.max(0, Math.floor((valorMinimo - padding) / 10000) * 10000)
  const yMax = Math.ceil((valorMaximo + padding) / 10000) * 10000
